...
from src.evaluation.ragas_calculator import calculate_ragas_metrics
...
def _generate_ground_truth_answer(test_case: UnifiedTestCase) -> str:
    """Generate expected answer using judge LLM based on ground truth data/vector.

    This function uses the SAME prompt logic as the main ReAct agent to ensure
    consistent evaluation criteria. The judge LLM generates what the answer SHOULD be
    from the ground truth data, then we compare it with the actual LLM response.

    Args:
        test_case: Test case with ground_truth_data and/or ground_truth_vector

    Returns:
        Expected answer generated by judge LLM
    """
    # Initialize judge LLM client (same model as main LLM for consistency)
    api_key = settings.gemini_api_key
    if not api_key:
        logger.warning("No Gemini API key found - using placeholder ground truth answer")
    ...
    sql_result = test_case.ground_truth_data or "No results"
    vector_result = test_case.ground_truth_vector or "No results"

    # Format results for readability
    sql_formatted = json.dumps(sql_result, indent=2) if sql_result != "No results" else "No results"
    vector_formatted = vector_result if vector_result != "No results" else "No results"
...
Your answer:"""
    ...
    try:
        ...
        # Fallback: return a descriptive message
        if sql_result != "No results":
            return f"Expected answer based on SQL data: {json.dumps(sql_result)[:200]}"
        elif vector_result != "No results":
            return f"Expected answer based on vector context: {vector_result[:200]}"
        else:
            return "Expected answer (no ground truth data available)"
    ...
    if start_index >= total_cases:
        ...
        with TestClient(app) as client:
            ...
            for i in range(start_index, total_cases):
                ...
                try:
                    # STEP 1: Generate ground truth answer using judge LLM
                    # Judge LLM uses the SAME prompt logic as the main LLM
                    logger.debug("Generating ground truth answer with judge LLM...")
                    ground_truth_answer = _generate_ground_truth_answer(test_case)
                    logger.debug(f"Ground truth answer: {ground_truth_answer[:100]}...")

                    # STEP 2: Build API request payload for actual LLM
                    payload = {
                    ...
                    # Calculate all 7 RAGAS metrics using actual answer, sources, and ground truth
                    ...
                    ragas_metrics = calculate_ragas_metrics(
                        ...
                        ground_truth_answer=ground_truth_answer,  # From judge LLM
                        ground_truth_vector=test_case.ground_truth_vector,  # From test case
                    )
