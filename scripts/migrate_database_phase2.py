"""
DATABASE MIGRATION - PHASE 2: Remove Redundant Columns
...
- player_stats: id, player_id, team_abbr, [42 stats]

AFTER:
- players: id, name, team_abbr, age (team column removed)
- player_stats: id, player_id, [42 stats] (team_abbr column removed)
"""
import shutil
import sqlite3
...
def import_data(db: NBADatabase, data: dict) -> None:
    ...
    try:
        ...
        # Import players (NOTE: no 'team' column in new schema)
        print("\n📥 Importing players...")
        for player_id, name, team_abbr, age in data['players']:
            # Get team name from teams table
            team_obj = db.get_team_by_abbreviation(session, team_abbr)
            team_name = team_obj.name if team_obj else f"Unknown ({team_abbr})"

            # NOTE: SQLAlchemy models still have 'team' field temporarily
            # Will be removed in next model update
            db.add_player(session, name, team_name, team_abbr, age)
        session.commit()
        print(f"   ✅ {len(data['players'])} players imported")

        # Import player_stats (NOTE: no 'team_abbr' column in new schema ideally)
        ...
        for row in data['player_stats']:
            ...
            # NOTE: team_abbr still exists in current model
            # Will be removed in next model update
            # For now, keep it
            # team_abbr = stats_dict.pop('team_abbr', None)
        ...
        from src.repositories.nba_database import DataDictionaryModel
...
def main():
    ...
    print(f"⚠️  Ensure you have completed Phase 1 first!")
    print(f"\n📂 Database: {db_path}")
